/**
 * @file visualize_aircraft_net_force.cpp
 * @brief 飞机净力数据可视化工具 - 通过调用MATLAB绘制图表
...
public:
    static std::string generatePlotScript(const CSVParser& parser, const std::string& output_dir) {
        ...
        for (size_t i = 2; i < headers.size(); ++i) { // 从第3列开始（跳过SimulationTime和datasource）
            std::string var_name = "data_" + std::to_string(i-1); // 从data_1开始
            script << var_name << " = [";
        ...
        int cols = 2;  // 2列布局
        int rows = (plot_count + 1) / 2;  // 自动计算行数
        ...
        // 保存图片
        std::string output_path = (fs::path(output_dir) / "aircraft_net_force.png").string();
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "========================================\n";
    std::cout << "飞机净力数据可视化工具 (MATLAB版本)\n";
    std::cout << "========================================\n\n";
    
    // 检查命令行参数
    if (argc < 2) {
        std::cout << "用法: " << argv[0] << " <CSV文件路径> [输出目录]\n";
        std::cout << "示例: " << argv[0] << " ../ScenarioExamples/B737_Taxi/output/aircraft_net_force.csv\n";
        return 1;
    }
    ...
    // 保存脚本文件
    std::string script_file = (fs::path(output_dir) / "plot_aircraft_net_force.m").string();
    ...
    if (MATLABCaller::callMATLAB(script_content, script_file)) {
        std::cout << "\n可视化完成!\n";
        std::cout << "输出文件:\n";
        std::cout << "- " << (fs::path(output_dir) / "aircraft_net_force.png").string() << std::endl;
    } else {
